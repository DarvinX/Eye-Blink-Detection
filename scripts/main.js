...
utils.loadOpenCv(()=>{
    ...
    function processVideo() {
        ...
        try {
            ...
            let eye_closed = faces.size() ? (eyes.size()+last_eye_size.reduce((a,b) => a+b)) ?false:true:null;

            console.log(eye_closed);
            for(let i = 0; i < filter_length;i++){

                last_eye_size[i] = ((i+1) == filter_length) ? eyes.size() : last_eye_size[i+1];
                //console.log(last_eye_size[i])
            }
